...
app
  .get("/", async (req, res) => {
    ...
    return res.send(`<ul>
    ${teamIds.map(
      (teamId) => `<li><a href="/teams/${teamId}">Team ${teamId}</li>`
    )}
    </ul>`);
  })
    ...
    try {
      for (let index = 0; index < req.body.events.length; index++) {
        ...
        if (
          ...
          event.message.text.startsWith("setup")
        ) {
          ...
          if (!teamId) {
            ...
            await lineClient.replyMessage(event.replyToken, {
              type: "text",
              text: `已設定 Team ID: ${teamId}`,
            });
            await postTeamStatus(teamId);
          }
        }
      }
      return res;
...
const fetchTeamArticles = async (teamId, force = false, page = 1) => {
  ...
  const articles = $(".team-article .ir-list")
    .map((_, el) => {
      ...
      const article = {
        id: articleId,
        author: $(el)
          .children(".ir-list__info")
          .children(".ir-list__user")
          .children("span.ir-list__name")
          .html(),
  ...
  if (!force) {
    const cachedTeamArticleKeys = await redisClient.keys(
...
const postTeamStatus = async (teamId) => {
  ...
  const message = {
    ...
    text:
      ...
      memberStatusList
        .map(
          (memberStatus) => `${memberStatus.member}: ${memberStatus.articles}`
        )
        .join("\n") +
      `\n\n團隊連結：\nhttps://ithelp.ithome.com.tw/2021ironman/signup/team/${teamId}`,
  };
  const receivers = await redisClient.smembers(`teams:${teamId}:receivers`);
  for (const receiver of receivers) {
    await lineClient.pushMessage(receiver, message);
  }
};
